...
sic_subdirs <- "/sic/csv/"
sic_prefix <- "sic_"
sic_body <- "_qtrly_by_area"
sic_extension <- ".zip"

naic_subdirs <- "/csv/"
naic_prefix <-  ""
naic_body <- "_qtrly_by_area"
naic_extension <- ".zip"

# this_year<- 1990
# test_sic <- "https://data.bls.gov/cew/data/files/1990/sic/csv/sic_1990_qtrly_by_area.zip"
# test_naic <- "https://data.bls.gov/cew/data/files/1990/csv/1990_qtrly_by_area.zip"
# 
# sic_filename <- paste0(sic_prefix,this_year, sic_body)
# sic_URL <- paste0(base_URL,this_year,sic_subdirs, sic_filename, sic_extension)
# sic_URL2 <- paste0(base_URL, this_year, sic_subdirs, sic_prefix, this_year, sic_body, sic_extension)
# 
# naic_filename <- paste0(naic_prefix, this_year, naic_body)
# naic_URL <- paste0(base_URL,this_year,naic_subdirs, naic_filename,   naic_extension)
# naic_URL2 <- paste0(base_URL,this_year, naic_subdirs, this_year, naic_body, naic_extension)
# 
# test_sic == sic_URL
# sic_URL == sic_URL2
...
vars_sic <- c("qtrly_estabs_count","month1_emplvl","month2_emplvl","month3_emplvl","avg_wkly_wage")

# Define extra variables needed
extra_sic_vars <- c("area_fips", "year","agglvl_code", "own_title",
                    "industry_title","qtr","industry_code")
  ...
  for (this_fips in test_fips){
    ...
    if(length(download_list$Name) == 0) {
      ...
      fips_vars <- subset(fips_df, select = c(extra_sic_vars, vars_sic)) %>% 
        filter(agglvl_code == 28 | agglvl_code == 29)  # %>% 
      ...
      # to compare with the countywide total. Do that in another program  
      # once this one has extracted everything.  
...
Download loop for NAIC data set years 1990-2020 (Present)
...
vars_naic <- c("qtrly_estabs_count","month1_emplvl","month2_emplvl","month3_emplvl","avg_wkly_wage")
  ...
  for (this_fips in test_fips){
    ...
    if(length(download_list$Name) == 0) {
      ...
      fips_vars <- subset(fips_df_naic, select = c(extra_naic_vars,  vars_naic)) %>% 
        filter(agglvl_code == 73 | agglvl_code == 74)  # %>% 
...
target_fips<- c( '01003','01097','02013','02016','02020','02050','02060','02070','02100','02105','02110','02122','02130',
           '02150','02164','02170','02180','02185','02188','02195','02198','02220','02230','02261','02270' ,'02275',
           '02282','06001','06013','06015','06023','06037','06041','06045','06053','06055','06059','06073', '06075', 
           '06079','06081','06083','06085','06087','06095','06097','06111','09001','09007','09009', '09011', '10001',
           '10003', '10005', '11001', '12005', '12009', '12011', '12015', '12017', '12019', '12021', '12029', '12031',
           ...
           '72069', '72071', '72075', '72079', '72087', '72089', '72091', '72095', '72097', '72103', '72109', '72111',
           '72113', '72115', '72117', '72119', '72123', '72127', '72133', '72137', '72143', '72145', '72147', '72151',
           '72153', '78010', '78020', '78030')

target_years  <- c(1975:2000)

#Check number of counties
# length(fips) # nchar(fips)
# 
# data("fips_codes")  # This really gets the "promise" of the data.  It doesn't appear until you use it.
# coastal_fips <- fips_codes %>%
#   mutate(fips_full = paste0(state_code, county_code)) %>%
#   filter(fips_full %in% fips) %>%
#   transmute(fips = fips_full, county, state)
# 
# colnames(coastal_fips)
# 
# saveRDS(coastal_fips, file = here("data","coastal_FIPS.RDS"))
